...
// (bug.0059). This fork logs all HTTP errors at warn (no bug.5056 operator-fault
// split); logger.error is mocked for completeness only.
...
describe("LiteLlmAdapter", () => {
  ...
  describe("bug.0059: private error diagnostics in operator logs", () => {
    ...
    it("returns [unreadable] when response body cannot be read", async () => {
      ...
      // operator-fault split), so assert against the warn channel.
      expect(mockLoggerWarn).toHaveBeenCalledWith(
        expect.objectContaining({
